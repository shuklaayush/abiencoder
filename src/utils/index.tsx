...
const validateArray = (value: string, itemValidator: Validator) => {
  try {
    ...
    if (!Array.isArray(parsedValue)) return VALIDATE_ERRORS.NOT_ARRAY;
...
const validators: { [x: string]: Validator } = {
  address: (value: string) =>
    utils.isAddress(value) ? null : VALIDATE_ERRORS.NOT_ADDRESS,
  "address[]": (value: string) => validateArray(value, validators.address),
  bool: (value: string) => {
    if (!["true", "false"].includes(value)) {
      return VALIDATE_ERRORS.NOT_BOOLEAN;
    }
    return null;
  },
...
export const encode = (parameters: Parameters) => {
  ...
  if (valid) {
    if (parameters.type !== "constructor") {
      const sig = parameters.name + "(" + types.join(",") + ")";
